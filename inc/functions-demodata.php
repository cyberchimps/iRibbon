...
function cyberchimps_demodata_style() {
	...
	wp_enqueue_style( 'about_style', get_template_directory_uri() . '/inc/css/options.css' );
	
	wp_enqueue_script( 'responsive_jquery_validation', get_stylesheet_directory_uri() . '/inc/js/jquery.validate.min.js', array(), true );
	wp_enqueue_script( 'responsive_jquery_validationform', get_stylesheet_directory_uri() . '/inc/js/validation.js', array(), true );
}
...
function iribbon_sender_email($sent_from)
{
	return $_POST['ccemail'];
}
function iribbon_mail_name ($sent_from)
{
	return $_POST['ccemail']; 
}
function cyberchimps_display_email() {
	...
	if (isset($_POST['ccSubmitBtn']))
	{
		...
		{
			$subject = "Demo Data Request for iRibbon";
			$headers = 'From: '.'<'.$_POST['ccemail'].'>'. "\r\n";			
			
			
			add_filter( 'wp_mail_from', 'iribbon_sender_email' );
			add_filter( 'wp_mail_from_name', 'iribbon_mail_name' );
...
<span class="ccinfo"><?php _e("Please enter your email ID and click on Send to receive the demo data for iRibbon.",'iribbon') ?></span>
