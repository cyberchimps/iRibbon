...
/* Define variables. */	
	if (is_page()) {
		...
		$tcolor = $options->get($ir_themeslug.'_blog_callout_text_color');
		$text = $options->get($ir_themeslug.'_blog_callout_text');
	}
...
/* Define Callout text. */	
	if ($text == '') {
		$callouttext = __( 'CyberChimps gives you the tools to turn WordPress into a modern feature rich Content Management System (CMS)', 'iribbon' );
	}
	else {
		$callouttext = $text;
	}
