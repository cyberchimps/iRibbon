...
function response_twitterbar_section_content() {
	...
	if ( is_page() ) {
		$handle = get_post_meta($post->ID, $ir_themeslug.'_twitter_handle' , true); 
		$replies = get_post_meta($post->ID, $ir_themeslug.'_twitter_reply' , true); 
		...
		$handle = $options->get($ir_themeslug.'_blog_twitter');
		$show_replies = $options->get($ir_themeslug.'_blog_twitter_reply');
	}
